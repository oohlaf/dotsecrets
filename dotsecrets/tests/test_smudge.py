...
class TestSmudgeFilter(unittest.TestCase):
    ...
    def test_nokey_sub(self):
        self.assertEqual(self.filters[0].sub('password = $DotSecrets: $ # comment'),
                                             'password = $DotSecrets: $ # comment')

    def test_nomatch_sub(self):
        self.assertEqual(self.filters[0].sub('password = $DotSecrets: notfound$ # comment'),
                                             'password = $DotSecrets: notfound$ # comment')

    def test_single_sub(self):
        self.assertEqual(self.filters[0].sub('password = $DotSecrets: password$ # comment'),
                                             'password = s3cr3t # comment')

    def test_double_sub(self):
        self.assertEqual(self.filters[0].sub('password = $DotSecrets: password$; security question = $DotSecrets: question$ # comment'),
